...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const categoryData = {
    ...
    zelte: {
      title: 'ZELTE',
      subtitle: 'Event-Pavillons & Zelte',
      sections: [
        {
          title: 'ZELTE',
          products: [
            {
              ...
              material: 'Wasserdichte PVC-Plane (B1 schwer entflammbar)',
              color: 'Weiß',
          ...
          products: [
            {
              ...
              img: 'images/VERKEHRSTECHNIK.jpeg',
              defaultQty: 10,
  ...
  function openCategorySubpage(catKey, pushState = true) {
    ...
    if (data) {
      ...
      // Dynamically hide material options that do not exist for active category products
      const matInputs = document.querySelectorAll('input[data-filter="material"]');
      const catProducts = [];
      data.sections.forEach(s => catProducts.push(...s.products));
      
      matInputs.forEach(input => {
        const lbl = input.closest('label');
        if (lbl) {
          const val = input.value.toLowerCase();
          const exists = catProducts.some(p => p.material && p.material.toLowerCase().includes(val));
          lbl.style.display = exists ? 'flex' : 'none';
        }
      });

      const matGroup = matInputs.length > 0 ? matInputs[0].closest('.filter-group') : null;
      if (matGroup) {
        const anyVisible = Array.from(matInputs).some(input => input.closest('label').style.display !== 'none');
        matGroup.style.display = anyVisible ? 'block' : 'none';
      }
    ...
    singleProductAnfragenBtn.addEventListener('click', () => {
      ...
      if (existing) {
        ...
        cart.push({ 
          ...
          articleNr: currentDetailProduct.articleNr || ('EV-' + currentDetailProduct.id.toUpperCase())
        });
      }
    ...
    container.querySelectorAll('.add-cart-btn').forEach(btn => {
      btn.onclick = (e) => {
        ...
        if (existing) {
          ...
          const prod = allProductsMap[id];
          const cardImg = card ? card.querySelector('img') : null;
          const img = prod ? prod.img : (cardImg ? cardImg.src : '');
          const articleNr = prod ? prod.articleNr : ('EV-' + id.toUpperCase());
          cart.push({ id, title, qty, img, articleNr });
        }
  ...
  function updateCartUI() {
    ...
    if (cart.length === 0) {
      ...
      cart.forEach((item, index) => {
        html += `
          <div class="cart-item" style="display: flex; align-items: center; justify-content: space-between; gap: 0.85rem; padding: 0.85rem 0; border-bottom: 1px solid #EAEAEA;">
            <div style="display: flex; align-items: center; gap: 0.85rem;">
              <div style="width: 52px; height: 52px; background: #FFFFFF; border: 1px solid #EAEAEA; border-radius: 6px; overflow: hidden; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                <img src="${item.img || 'images/Eventnight-Logo-ausgeschnitten.png'}" alt="${item.title}" style="max-width: 100%; max-height: 100%; object-fit: contain;">
              </div>
              <div>
                <div class="cart-item-title" style="font-weight: 800; font-size: 0.92rem; color: #111; font-family: var(--font-heading);">${item.title}</div>
                <div class="qty-picker" style="margin-top: 0.35rem;">
            ...
            <button class="cart-item-remove" data-index="${index}" title="Entfernen" style="font-size: 1.05rem; color: #EF4444; background: none; border: none; cursor: pointer; padding: 0.4rem;">
